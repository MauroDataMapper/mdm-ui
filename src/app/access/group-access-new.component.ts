...
export class GroupAccessNewComponent implements OnInit {
    ...
    loadAllGroups(text, offset, limit) {
        ...
        this.securityHandler.isValidSession().subscribe((result) => {
            ...
            this.resourceService.userGroup.get().subscribe((data) => {
    ...
    buildGroups = function() {
        ...
        for (let i = 0; this.folderResult.writeableByGroups && i < this.folderResult.writeableByGroups.length; i++) {
            let group = this.folderResult.writeableByGroups[i];
        ...
        for (let i = 0; this.folderResult.readableByGroups && i < this.folderResult.readableByGroups.length; i++) {
            let group = this.folderResult.readableByGroups[i];
    ...
    save(row, index) {
        ...
        let gId = row.edit.group.id;

        if (row.readAccess && !row.edit.readAccess) {
            observableCol.push(this.resourceService[name].delete(mId, 'read/group/' + gId));

        } else {
            // Put WriteAccess
            if (!row.writeAccess && row.edit.writeAccess) {
                observableCol.push(this.resourceService[name].put(mId, 'write/group/' + gId));
            } else {
                // Delete WriteAccess
                if (row.writeAccess && !row.edit.writeAccess) {
                    observableCol.push(this.resourceService[name].delete(mId, 'write/group/' + gId));
                } else if (!row.readAccess && row.edit.readAccess) {
                    // Put ReadAccess
                    observableCol.push(this.resourceService[name].put(mId, 'read/group/' + gId));
                }
            }
        }

        Observable.forkJoin(observableCol).subscribe((result) => {
    ...
    onGroupSelect = function(select, record) {
