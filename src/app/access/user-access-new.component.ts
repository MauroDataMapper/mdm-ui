...
export class UserAccessNewComponent implements OnInit, AfterViewInit {
    ...
    buildUsers() {
        ...
        for (let i = 0; this.folderResult.writeableByUsers && i < this.folderResult.writeableByUsers.length; i++) {
            let user = this.folderResult.writeableByUsers[i];
        ...
        for (let i = 0; this.folderResult.readableByUsers && i < this.folderResult.readableByUsers.length; i++) {
            let user = this.folderResult.readableByUsers[i];
    ...
    save(row, index) {
        ...
        let observableCol = [];
        let name = this.supportedDomainTypes[this.parentType].name;
        let message = this.supportedDomainTypes[this.parentType].message;

        let mId = this.folderResult.id;
        ...
        if (row.edit.user && row.edit.user.id !== -1) {
            let uId = row.edit.user.id;
            // Delete ReadAccess
            if (row.readAccess && !row.edit.readAccess) {
                observableCol.push(this.resourceService[name].delete(mId, 'read/user/' + uId));
            } else {
                // Put WriteAccess
                if (!row.writeAccess && row.edit.writeAccess) {
                    observableCol.push(this.resourceService[name].put(mId, 'write/user/' + uId));
                } else {
                    // Delete WriteAccess
                    if (row.writeAccess && !row.edit.writeAccess) {
                        observableCol.push(this.resourceService[name].delete(mId, 'write/user/' + uId));
                    } else if (!row.readAccess && row.edit.readAccess) {
                        // Put ReadAccess
                        observableCol.push(this.resourceService[name].put(mId, 'read/user/' + uId));
                    }
                }
            }
        } else {
            ...
            if (row.edit.writeAccess) {
                observableCol.push(this.resourceService[name].put(mId, 'write/user/', { resource }));
            } else if (!row.edit.writeAccess && row.edit.readAccess) {
                // if it's Write, then do not call read
                observableCol.push(this.resourceService[name].put(mId, 'read/user/', { resource }));
            }

        }

        Observable.forkJoin(observableCol).subscribe((results: any) => {
            row.readAccess = row.edit.readAccess;
