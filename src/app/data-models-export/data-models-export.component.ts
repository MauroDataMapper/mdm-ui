...
export class DataModelsExportComponent implements OnInit {
  ...
  constructor(private changeDedRef: ChangeDetectorRef, private securityHandler: SecurityHandlerService, private resources: ResourcesService, private messageHandler: MessageHandlerService, private exportHandler: ExportHandlerService, private helpDialogueHandler: HelpDialogueHandlerService, private renderer: Renderer2) {
  ...
  export () {
    ...
    this.exportHandler.exportDataModel(this.selectedDataModels, this.selectedExporterObj).subscribe(result => {
     if (result != null) {
      ...
      var label = this.selectedDataModels.length === 1 ? this.selectedDataModels[0].label : 'data_models';
      var fileName = this.exportHandler.createFileName(label, this.selectedExporterObj);
      var file = new Blob([ result.body ], {type : this.selectedExporterObj.fileType});
      var link = this.exportHandler.createBlobLink(file, fileName);
      Array.from(this.aLink.nativeElement.children).forEach(child => {
          this.renderer.removeChild(this.aLink.nativeElement, child);
       });
