...
import {DataElementResult , EditableDataElement} from '../../model/dataElementModel';
...
export class DataElementDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.editableForm.show = () => {
      this.editForm.forEach(x => x.edit({ editing: true, focus: x._name === 'moduleName' ? true : false }));
...
if (this.validateLabel(this.result.label) && this.validateMultiplicity(this.min, this.max)) {
      ...
      let call = from(this.resourcesService.dataElement.put(this.parentDataModel.id, this.parentDataClass.id, resource.id, null, { resource })).subscribe(result => {
...
validateMultiplicity(minVal, maxVal) {
    ...
    let errorMessage = this.validator.validateMultiplicities(min, max);
    if (errorMessage) {
