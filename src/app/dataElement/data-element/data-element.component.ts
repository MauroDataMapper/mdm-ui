...
import {DataElementResult} from "../../model/dataElementModel";
...
export class DataElementComponent implements OnInit {
  ...
  dataElementDetails(dataModelId: any, dataClassId,id) {
    this.resourcesService.dataElement.get(dataModelId, dataClassId, id, null, null).subscribe((result: { body: DataElementResult; }) => {
  ...
  getTabDetailByIndex(index) {
    switch (index) {
      ...
      case 4: return { index: 4, name: 'attachments' };
  ...
  tabSelected(index) {
    ...
    this.stateHandler.Go("dataElement", { tabView: tab.name }, { notify: false, location: tab.index != 0 });
