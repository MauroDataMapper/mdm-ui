...
export class DataModelComponent implements OnInit {
    ...
    dataModelDetails(id: any) {
        this.resourcesService.dataModel.get(id, null, null).subscribe((result: { body: DataModelResult; }) => {
            this.dataModel = result.body;

            this.parentId = this.dataModel.id;
            this.showExtraTabs = !this.sharedService.isLoggedIn || (!this.dataModel.editable || this.dataModel.finalised);
  ...
  DataModelPermissions(id: any) {
    this.resourcesService.dataModel.get(id, 'permissions', null).subscribe((permissions: {body: {[x: string]: any; }; }) => {
    ...
    getTabDetailByIndex(index) {
        switch (index) {
        case 0: return { index: 0, name: 'dataClasses' };
        case 1: return { index: 1, name: 'types' };
        case 2: return { index: 2, name: 'properties' };
        case 3: return { index: 3, name: 'summaryMetadata' };
        case 4: return { index: 4, name: 'comments' };
        case 5: return { index: 5, name: 'history' };
        case 6: return { index: 6, name: 'diagram' };
        case 7: return { index: 7, name: 'links' };
        case 8: return { index: 8, name: 'attachments' };
        case 9: {
            if (this.dataModel.type === 'Data Asset') {
                return { index: 9, name: 'dataflow' };
            }
    ...
    tabSelected(index) {
        let tab = this.getTabDetailByIndex(index);
