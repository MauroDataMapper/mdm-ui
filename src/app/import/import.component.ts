...
export class ImportComponent implements OnInit {
    ...
    constructor(private title: Title, private resources: ResourcesService, private messageHandler: MessageHandlerService, private helpDialogueHandler: HelpDialogueHandlerService, private stateHandler: StateHandlerService, private broadcastSvc: BroadcastService) { }
    ...
    loadImporters = (multiple?) => {
        this.resources.public.dataModelImporterPlugins(multiple)
            .subscribe((result) => {
    ...
    loadImporterParameters = (selectedItem) => {
        ...
        this.resources.importer.get(action).subscribe((res) => {
            ...
            for (var g = 0; g < this.selectedImporterGroups.length; g++) {
                var parameters = this.selectedImporterGroups[g].parameters;
    ...
    startImport = () => {
        ...
        var namespace = this.selectedImporterObj.namespace;
        var name = this.selectedImporterObj.name;
        var version = this.selectedImporterObj.version;
        ...
        this.resources.dataModel.import(namespace + "/" + name + "/" + version, this.formData).subscribe((result:any) => {
            ...
            this.messageHandler.showSuccess("Data Model(s) imported successfully");
            this.broadcastSvc.broadcast('$reloadDataModels');

            if (result && result.body.count === 1) {
                this.stateHandler.Go("datamodel", { id: result.body.items[0].id }, { reload: true, location: true });
            }
    ...
    getFile = (paramName) => {
        var element : any = document.getElementById(paramName);
