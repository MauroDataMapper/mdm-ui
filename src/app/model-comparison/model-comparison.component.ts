...
export class ModelComparisonComponent implements OnInit {
    ...
    async ngOnInit() {
        ...
        if (sourceId && targetId) {
            const source = await this.loadDataModelDetail(sourceId).toPromise();
            this.sourceModel = source.body;
            const sourceChildren = await this.loadModelTree(this.sourceModel ).toPromise();
            this.sourceModel.children = sourceChildren.body;
            if (this.sourceModel.children?.length > 0) {
                this.sourceModel.hasChildren = true;
            }
            this.sourceModel.close = false;

            const target = await this.loadDataModelDetail(targetId).toPromise()
            this.targetModel = target.body;
            const targetChildren = await this.loadModelTree(this.targetModel).toPromise();
            this.targetModel.children = targetChildren.body;
            if (this.targetModel.children?.length > 0) {
                this.targetModel.hasChildren = true;
            }
