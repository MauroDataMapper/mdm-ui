...
export class FavouriteHandlerService {
  ...
  add(element) {
    var favorites = this.userSettingsHandler.get("favourites");
    var fvt = false;
    ...
    this.userSettingsHandler.saveOnServer().subscribe(result => {
      ...
      this.broadcastSvc.broadcast("favourites", {name: "add", element: element});
    },error => {
  ...
  remove(element) {
    ...
    this.userSettingsHandler.saveOnServer().subscribe(result =>{
      ...
      this.broadcastSvc.broadcast("favourites", { name: "remove", element: element});
      //deferred.resolve(favorites);
    },(error) => {
      this.messageHandler.showError('There was a problem updating the Favorites.', error);
