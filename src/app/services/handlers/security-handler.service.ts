...
export class SecurityHandlerService {
    ...
    constructor(private elementTypes : ElementTypesService, private resources: ResourcesService, private cookies: CookieService, private stateHandler: StateHandlerService) { }
    ...
    getUserFromCookie() {
        if (this.cookies.get('username') && this.cookies.get('username').length > 0) {
            return {
    ...
    login(username, password) {
        ...
        var promise = new Promise((resolve, reject) => {
            this.resources.authentication.post("login",
                { resource: resource },
                { login: true, ignoreAuthModule: true, withCredentials: true }).subscribe((res) => {
                    var result = res.body;
                    var currentUser = {
                        id: result.id,
                        token: result.token,
    ...
    logout() {
       return this.resources.authentication.post("logout", null, { responseType :'text'}).subscribe(result => {
            this.removeCookie();
    ...
    showIfRoleIsWritable(element) {
        ...
        var isEditable = environment.appIsEditable;
    ...
    elementAccess(element) {
        ...
        var dataTypes = this.elementTypes.getAllDataTypesMap();
