...
export class HelpDialogueHandlerService {
    ...
    open(name: string, position: DialogPosition): void {
        ...
        let contentWikiLink: any = wikiLink + '&action=render';

        let safeWikiLink: any = this.sanitizer.bypassSecurityTrustResourceUrl(contentWikiLink);

        const dg = this.dialog.open(HelpDialogComponent,
            {
                hasBackdrop: false,
                minHeight: '500px',
                minWidth: '400px',
