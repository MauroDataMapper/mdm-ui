...
export class ContentTableComponent implements AfterViewInit {
  ...
  constructor(private messageHandler: MessageHandlerService, private resources: ResourcesService, private stateHandler: StateHandlerService, private changeRef: ChangeDetectorRef) { }
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.filterEvent)
        .pipe(
            ...
            switchMap(() => {
                  ...
                  return this.contentFetch(this.paginator.pageSize,
  ...
  deleteRows = () => {
    ...
    this.records.forEach((record) => {
      ...
      if (record.domainType === 'DataClass') {
        ...
        chain.push(
            this.resources.dataElement.delete(record.dataModel, record.dataClass, record.id).catch((error) => {
              this.failCount++;
           })
        );
      }
