...
export class DataElementsListComponent implements AfterViewInit  {
  ...
  loadDataClasses = () => {
    merge(this.sort.sortChange, this.paginator.page, this.filterEvent).pipe(startWith({}), switchMap(() => {
      ...
      return this.dataElementsFetch(this.paginator.pageSize, this.paginator.pageOffset, this.sort.active, this.sort.direction, this.filter);
    }), map((data: any) => {
  ...
  updateDataElementsOrder = (item, newPosition) => {
    ...
    this.resources.dataElement.update(this.parentDataModel.id, this.parentDataClass.id, resource.id, resource).subscribe(result => {
