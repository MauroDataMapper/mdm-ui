...
export class ElementDataTypeComponent implements OnInit {
    ...
    constructor(private stateHandler: StateHandlerService, private elementTypes: ElementTypesService) { }
    ...
    ngOnInit() {
        ...
        if (this.elementDataType !== null && this.elementDataType !== undefined) {
            ...
            var parentDataModelId = this.mcParentDataModel ? this.mcParentDataModel.id : null;
            if (!parentDataModelId) {
                parentDataModelId = this.elementDataType.dataModel;
            }

            if (this.elementDataType.domainType === 'ReferenceType' && this.elementDataType.referenceClass) {
                this.referenceClass = this.elementDataType.referenceClass;
                this.referenceClassLink = this.stateHandler.getURL("dataclass", { id: this.elementDataType.referenceClass.id, dataModelId: parentDataModelId });
            }
        ...
        if (this.elementDataType.enumerationValues !== null) {
            ...
            if (this.elementDataType && this.elementDataType.domainType === 'EnumerationType') {
                ...
                var categories = from(this.allRecords).pipe(groupBy(record => record.category), mergeMap(group => group.pipe(toArray())));
                ...
                categories.subscribe((categories: Categories[]) => {
                    ...
                    categoryNames.forEach((category) => {
                        ...
                        categories.filter(x => x.category === category).forEach((row) => {
    ...
    showMore = (element: any) => {
        if (this.showMoreIcon && !this.showing) {
            var elements = element.parentElement.offsetParent.getElementsByClassName("moreEnumerationKeyValue")
            for (var i = 0; i < elements.length; i++) {
                elements[i].classList.remove("hiddenMoreEnumerationKeyValue");
            }
            element.innerHTML = "hide <span class='fa fa-caret-up'></span>";
        } else {
            var elements = element.parentElement.offsetParent.getElementsByClassName("moreEnumerationKeyValue")
            for (var i = 0; i < elements.length; i++) {
                elements[i].classList.add("hiddenMoreEnumerationKeyValue");
            }
            element.innerHTML = "... more <span class='fa fa-caret-down'></span>";
        }
