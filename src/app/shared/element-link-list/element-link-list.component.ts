...
import { SemanticLinkHandlerService } from '../../services/handlers/semantic-link-handler.service';
...
export class ElementLinkListComponent implements AfterViewInit {
    ...
    constructor(public elementTypes: ElementTypesService,
        ...
        merge(this.sort.sortChange, this.paginator.page, this.filterEvent)
            .pipe(
                ...
                switchMap(() => {
                    ...
                    return this.semanticLinkFetch(this.paginator.pageSize,
                ...
                for (let i = 0; i < data.length; i++) {
                    data[i].status = data[i].source.id === this.parent.id ? 'source' : 'target';
                }
    ...
    semanticLinkFetch = (pageSize, pageIndex, sortBy, sortType, filters) => {
        ...
        if (this.parent.domainType === 'Term') {
            return this.resources.term.get(this.terminology, this.parent.id, 'semanticLinks', options);
        } else {
            return this.resources.catalogueItem.get(this.parent.id, 'semanticLinks', options);
        }
    }
    ...
    delete = (record, $index) => {
        ...
        this.resources.catalogueItem.delete(this.parent.id, 'semanticLinks', record.id).subscribe(
            () => {
                if (this.type === 'static') {
    ...
    save = (record, index) => {
        ...
        if (record.id && record.id !== '') {
            ...
            this.semanticLinkHandler.put(this.parent, record.edit.target, record.id, record.edit.linkType)
                .subscribe((res) => {
                    if (this.afterSave) {
                ...
                .catch(function(error) {
                    this.messageHandler.showError('There was a problem updating the link.', error);
                });
        } else {
    ...
    findElement = (record) => {
        let domainTypes = [];
        let notAllowedToSelectIds = [this.parent.id];
        ...
        let dataTypes = this.elementTypes.getAllDataTypesMap();
