import { Component, AfterViewInit, Input, OnChanges, ViewChild, ViewChildren, QueryList, ElementRef, EventEmitter, ChangeDetectorRef } from '@angular/core';
        ...
        if (select) {
            ...
            for (let i = 0; i < this.namespaces.length; i++) {
                if (this.namespaces[i].namespace === select.namespace) {
                    ...
                    let id = 0;
                    record.metadataKeys = this.namespaces[i].keys.map((key) => {
                        return { id: id++, key };
                    });
                    break;
                }
