...
export class McSelectComponent implements OnInit, AfterViewInit, OnDestroy {
        ...
        if (this.defaultValue) {
            ...
            if (this.multiSelect === true) {
                ...
                if (typeof this.defaultValue === 'object') {
                    this.selectedValue = this.defaultValue;
                    let value = this.validator.getProperty(this.selectedValue, this.displayProperty);
    ...
    onInputKeyDown(event) {
        ...
        if (event.keyCode === 27) {
            ...
            let currentElements = this.mcSelectHolder.nativeElement.querySelectorAll('#current');


            if (currentElements.length > 0) {
                let nextElement = this.elements._results[this.elements._results.findIndex(x => x.nativeElement === currentElements[0]) + 1];
                ...
                let elPosition = this.elementPositionInHolder(nextElement);
                let elHeight = nextElement.nativeElement.offsetHeight;
                let holderHeight = this.mcSelectHolder.nativeElement.offsetHeight;
            ...
            if (currentElements.length > 0) {
                let prevElement = this.elements._results[this.elements._results.findIndex(x => x.nativeElement === currentElements[0]) - 1];
                ...
                let elPosition = this.elementPositionInHolder(prevElement);
                let holderScrollTop = this.mcSelectHolder.nativeElement.scrollTop;
            ...
            let currentElement = this.mcSelectHolder.nativeElement.querySelectorAll('.current');
    ...
    elementPositionInHolder(el) {
        let fromTop = 0;
        let indexOfElement = this.elements._results.findIndex(x => x === el);
	...
	filterValues(inputValue) {
        ...
        let p = new Promise((resolve, error) => {
            ...
            if (this.valueType === 'static') {
             ...
             let query = this.loadDynamicValues(inputValue, loadAll, this.pagination.offset, this.pagination.limit);
    ...
    loadMore() {
        ...
        if (this.valueType === 'dynamic' && this.loadingDynamicData === false) {
            ...
            let loadedSoFar = this.pagination.offset + this.pagination.limit;
