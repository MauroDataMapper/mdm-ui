...
export class DataTypeStep2Component implements OnInit {
	...
	ngOnInit() {
		...
		this.dataSourceSelectedDataTypes = new MatTableDataSource<any>(this.model.selectedDataTypes);
	}
	...
	dataTypesFetch(pageSize, pageIndex, sortBy, sortType, filters) {
		...
		return this.resourceService.dataModel.get(this.model.copyFromDataModel[0].id, 'dataTypes', options);
	}
				...
				merge(this.sort.toArray()[0].sortChange, this.paginator.toArray()[0].page, this.filterEvent)
					.pipe(
						...
						switchMap(() => {
								...
								return this.dataTypesFetch(this.paginator.toArray()[0].pageSize,
	...
	reCheckSelectedDataTypes() {
		...
		this.model.selectedDataTypes.forEach((sdt: any) => {
			...
			let currentId = sdt.id;
			let item = this.recordsDataTypes.find(r => r.id === currentId);
	...
	createSelectedArray = () => {
		...
		this.model.selectedDataTypes = [];
		for (let id in this.model.selectedDataTypesMap) {

			if (this.model.selectedDataTypesMap.hasOwnProperty(id)) {

				let element = this.model.selectedDataTypesMap[id];
	...
	onCheckAll = () => {
		...
		for (let i = 0; i < this.recordsDataTypes.length; i++) {
			...
			if (this.checkAllCheckbox) {
				...
				let currentId = this.recordsDataTypes[i].id;
				let index = this.model.selectedDataTypes.findIndex(function(r) {
	...
	async saveCopiedDataTypes() {
		...
		this.model.selectedDataTypes.forEach(async (dt: any) => {
			...
			let action = 'dataTypes/' + dt.dataModel + '/' + dt.id;

			await this.resourceService.dataModel.post(this.model.parent.id, action, null).toPromise().then((result) => {
				...
				(error) => {
					this.failCount++;
					let errorText = this.messageHandler.getErrorText(error);
					this.finalResult[dt.id] = { result: errorText, hasError: true };
				});
		});
	}
