...
export class ReferenceDataTypeStep2Component implements OnInit {
  ...
  get parent() {
    return this.step.scope.parent;
  }

  get state() {
    return this.step.scope.state;
  }

  get model() {
    return this.step.scope.state.copyFromModel?.[0];
  }
  ...
  newDataTypeFormChanged(event: NewReferenceDataTypeState) {
    this.state.newDataTypeDetails = event;
    this.step.invalid = !event.valid;
  }

  dataTypesForCopySelected(event: ReferenceDataType[]) {
    this.state.dataTypesForCopy = event;
    this.step.invalid = !event || event.length === 0;
  }
}
